...
smoothing=b01       # vertical smoothing (km), valid are (see "man grdfilter"):
                    # (b)oxcar, (c)osine Arch, (g)aussian, (m)edian,
                    # or maximum likelihood (p)robability.
xsmoothratio=10      # horizontal/vertical smoothing ratio (km)
width=12            # plot width in inch; if equals 0, then 1:1 scale with
                    # height value given below is used.
height=2            # plot height in inch; if equals 0, then 1:1 scale with
                    # width given above is used.
x_axis=lat          # choose one from (lat, lon, km).
data=MISSEC.DAT     # data
data1=MISSEC_1.DAT     # data
input=missec07.inp  # input for missec07
ps=profile_gg_color.ps       # output postscript file
stalocal=../GG_STATION      # Plotting Station Location
...
xyz2grd data.xyz -Gmissec.grd -I$i -R$r
#surface data.xyz -Gmissec.grd -I0.05/0.1 -R$r
grdimage missec.grd -C$cpt -J$j -K -O -Y2.0 >> $ps
psbasemap -R$startla/$endla/$dep1/$dep2 -JX$width/-$height -Ba0.2f0.1/a20f10:"Depth (km) ":Wnse -K -O < /dev/null >> $ps


		else
		if [ $x_axis = "km" ] 
			then
			psbasemap -R$r -J$j -Ba50f25g50:"Distance (km)":/a20f10g20:"Depth (km)":WseN -K -O < /dev/null >> $ps
			else
...
project -C$startlo/$startla -E$endlo/$endla -G2/90 -Q| grdtrack -G../China.grd -o1,3 | awk -v var=$endla '{print $0};END{print var" 0"}' |psxy -JX$width/0.25i -R$startla/$endla/0/7000 -Ba0.2/a4000f2000:"H (m)":Ws -W0.5 -G230 -Y2i -K -O  >> $ps


cat $stalocal |while read line; do grep ^$line $stainfo  >> sta.tmp; done

cat sta.tmp |while read line; do proj=`echo $line | awk '{print $3,$2}' | mapproject -LLine_GG |awk '{print $4,$5}'`;proji=`echo $line|awk '{print $4,$1}'`; echo "$proj $proji" >> staproj.tmp; done
